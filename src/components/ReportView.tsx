...
export default function ReportView({ data, onBack }: { data: MonthData; onBack: () => void }) {
  ...
  const totalIncome = (data.incomes || []).reduce((s, e) => s + (e.amount || 0), 0);
  ...
  return (
    <div>
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        <button onClick={handleDownload} className="flex-1 flex items-center justify-center gap-2 text-xl px-5 py-3 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700">
          <Download size={20} /> ডাউনলোড করুন
        </button>
      </div>

      <div ref={printRef} className="bg-white border border-gray-200 rounded-2xl p-6 max-w-md mx-auto">
        ...
        {(data.incomes || []).map((e, i) => <Row key={i} label={e.name} value={fmtBDT(e.amount)} />)}
        <Row label="মোট" value={fmtBDT((data.opening || 0) + totalIncome)} bold />
